...
public class Account {
    ...
    private LDLinkedList<Message> messagesIn;
    private LDLinkedList<Message> messagesOut;
    ...
    public Account(int accId, String uName, String bDate, String loc){
        ...
        birthdate = bDate;
        location = loc;
        following = new LDLinkedList<String>(); // allocating memory for string.
        followers = new LDLinkedList<String>();
        messagesIn = new LDLinkedList<Message>();
        messagesOut = new LDLinkedList<Message>();  
        blockedAccounts = new LDLinkedList<Account>();
        posts = new LDLinkedList<Post>();
        UserActions = new LDLinkedList<String>();
        accountId = accId;
    }
    ...
    public void login(LDLinkedList <Account> acc){ // Revise it later.
        for(Account a : acc){
            if(a.isLoggedIn()){
    ...
    public void message(String content, Account other){
        ...
        if(!isFollower){
            System.out.println("You can't message this user, because you don't follow him/her!");
            return;
        }
    ...
    public void viewPostInteractions(Account profile, LDLinkedList <Account> arr){
        if(!isLoggedIn()){
     ...
     * @param acc Account array containing all the users that have been created.
     */
    public void viewInbox(LDLinkedList <Account> acc){
    ...
    public void addLike(Post p, LDLinkedList<Account> acc){
        if(!isViewedPost) {
        ...
        for(Account a: acc){
            if(a.accountId == p.accountId) {
                UserActions.add("You liked " + a.username + "'s post id: " + p.PostId ); // will be reviewed.
            }
        }
     ...
     * @param acc Account array containing all the users that have been created.
     */
    public void addComment(Post p, String text, LDLinkedList<Account> acc){
        if(!isViewedPost) {
        ...
        for(Account a: acc){
            if(a.accountId == p.accountId) {
                UserActions.add("You commented on " + a.username + "'s post id: " + p.PostId );
            }
        }
